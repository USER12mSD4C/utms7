...
int main(int argc, char **argv) {
    ...
    if (strcmp(cmd, "lsblk") == 0 || strcmp(cmd, "disks") == 0) return cmd_lsblk(argc, argv);
    if (strcmp(cmd, "mount") == 0) return cmd_mount(argc, argv);
    if (strcmp(cmd, "umount") == 0) return cmd_umount(argc, argv);
    if (strcmp(cmd, "mkfs.ufs") == 0) return cmd_mkfs_ufs(argc, argv);

    out("usfutils: unknown command: ");
    out(cmd);
    out("\n");
