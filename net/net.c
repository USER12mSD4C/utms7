...
void net_init(void) {
    ...
    if (dev) {
        vga_write("Found Intel e1000, initializing...\n");
        if (e1000_init(dev) == 0) {
            e1000_get_mac(our_mac);
            network_ready = 1;
            vga_write("Intel e1000 driver loaded\n");
        }
    }
    ...
    if (!dev) {
        ...
        if (dev) {
            vga_write("Found Realtek RTL8139, initializing...\n");
            if (rtl8139_init(dev) == 0) {
                rtl8139_get_mac(our_mac);
                network_ready = 1;
                vga_write("RTL8139 driver loaded\n");
            }
        }
    }
    ...
    vga_write("MAC: ");
    for (int i = 0; i < 6; i++) { vga_write_hex(our_mac[i]); if (i < 5) vga_write(":"); }
    vga_write("\n");
    ...
    if (network_ready) {
        ...
        for (int attempt = 0; attempt < 3; attempt++) {
            ...
            if (our_ip != 0) break;
            vga_write("DHCP attempt "); vga_write_num(attempt + 1); vga_write(" failed\n");
        }
    }
    ...
    vga_write("IP:     "); vga_write_num((our_ip >> 24) & 0xFF); vga_write(".");
    vga_write_num((our_ip >> 16) & 0xFF); vga_write("."); vga_write_num((our_ip >> 8) & 0xFF);
    vga_write("."); vga_write_num(our_ip & 0xFF); vga_write("\n");
    
    vga_write("Gateway: "); vga_write_num((our_gateway >> 24) & 0xFF); vga_write(".");
    vga_write_num((our_gateway >> 16) & 0xFF); vga_write("."); vga_write_num((our_gateway >> 8) & 0xFF);
    vga_write("."); vga_write_num(our_gateway & 0xFF); vga_write("\n");
    
    vga_write("Netmask: "); vga_write_num((our_netmask >> 24) & 0xFF); vga_write(".");
    vga_write_num((our_netmask >> 16) & 0xFF); vga_write("."); vga_write_num((our_netmask >> 8) & 0xFF);
    vga_write("."); vga_write_num(our_netmask & 0xFF); vga_write("\n");
    
    vga_write("DNS:     "); vga_write_num((our_dns >> 24) & 0xFF); vga_write(".");
    vga_write_num((our_dns >> 16) & 0xFF); vga_write("."); vga_write_num((our_dns >> 8) & 0xFF);
    vga_write("."); vga_write_num(our_dns & 0xFF); vga_write("\n");
    vga_write("===========================\n\n");
...
void net_eth_send(u8 *dst_mac, u16 type, u8 *data, int len) {
    ...
    if (e1000_present()) e1000_send(packet, len + sizeof(eth_hdr_t));
    else if (rtl8139_present()) rtl8139_send(packet, len + sizeof(eth_hdr_t));
    kfree(packet);
}
