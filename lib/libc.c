...
void *pci_map(int bus, int slot, int func, int bar) {
    long res = syscall(uSYS_pci_map, bus, slot, func, bar, 0, 0);
    return (res == -1) ? NULL : (void*)res;
}

void pci_unmap(void *addr, size_t size) {
    syscall(uSYS_pci_unmap, (long)addr, size, 0, 0, 0, 0);
}

int irq_register(int irq) {
    return syscall(uSYS_irq_register, irq, 0, 0, 0, 0, 0);
}

int irq_wait(int irq) {
    return syscall(uSYS_irq_wait, irq, 0, 0, 0, 0, 0);
}

u8 ioport_in8(u16 port) {
    return syscall(uSYS_ioport_in, port, 1, 0, 0, 0, 0);
}

u16 ioport_in16(u16 port) {
    return syscall(uSYS_ioport_in, port, 2, 0, 0, 0, 0);
}

u32 ioport_in32(u16 port) {
    return syscall(uSYS_ioport_in, port, 4, 0, 0, 0, 0);
}

void ioport_out8(u16 port, u8 val) {
    syscall(uSYS_ioport_out, port, val, 1, 0, 0, 0);
}

void ioport_out16(u16 port, u16 val) {
    syscall(uSYS_ioport_out, port, val, 2, 0, 0, 0);
}

void ioport_out32(u16 port, u32 val) {
    syscall(uSYS_ioport_out, port, val, 4, 0, 0, 0);
}
...
int rename(const char *old, const char *new_path) { return syscall(SYS_rename, (long)old, (long)new_path, 0, 0, 0, 0); }
int chdir(const char *path) { return syscall(SYS_chdir, (long)path, 0, 0, 0, 0, 0); }
int fs_register(const char* name) { return syscall(uSYS_fs_register, (long)name, 0, 0, 0, 0, 0); }
...
int execve(const char *path, char *const argv[], char *const envp[]) { return syscall(SYS_execve, (long)path, (long)argv, (long)envp, 0, 0, 0); }
int execvp(const char *file, char *const argv[]) { return execve(file, argv, NULL); }
int waitpid(int pid, int *status, int options) { return syscall(SYS_wait4, pid, (long)status, options, 0, 0, 0); }
...
unsigned int sleep(unsigned int seconds) { syscall(uSYS_sleep, seconds * 1000, 0, 0, 0, 0, 0); return 0; }
...
time_t time(time_t *tloc) {
    time_t t = (time_t)(syscall(uSYS_gettime, 0, 0, 0, 0, 0, 0) / 1000);
    if (tloc) *tloc = t;
    return t;
}
unsigned int getticks(void) { return syscall(uSYS_gettime, 0, 0, 0, 0, 0, 0); }
...
ssize_t send(int sockfd, const void *buf, size_t len, int flags) { return syscall(SYS_sendto, sockfd, (long)buf, len, flags, 0, 0); }
ssize_t recv(int sockfd, void *buf, size_t len, int flags) { return syscall(SYS_recvfrom, sockfd, (long)buf, len, flags, 0, 0); }
int close_socket(int sockfd) { return syscall(SYS_close, sockfd, 0, 0, 0, 0, 0); }

unsigned int gethostbyname(const char *name) {
    unsigned int ip;
    long res = syscall(uSYS_gethostbyname, (long)name, (long)&ip, 0, 0, 0, 0);
    return res == 0 ? ip : 0;
}

void clear_screen(void) { syscall(uSYS_clear, 0, 0, 0, 0, 0, 0); }
void set_color(int fg, int bg) { syscall(uSYS_setcolor, fg, bg, 0, 0, 0, 0); }

int disk_list(void* disks, int max) { return syscall(uSYS_disk_list, (long)disks, max, 0, 0, 0, 0); }
int partition_mount(const char* dev, const char* point) { return syscall(uSYS_part_mount, (long)dev, (long)point, 0, 0, 0, 0); }
int partition_umount(void) { return syscall(uSYS_part_umount, 0, 0, 0, 0, 0, 0); }
int partition_format(const char* dev, const char* fstype) { return syscall(uSYS_part_format, (long)dev, (long)fstype, 0, 0, 0, 0); }
int disk_create_table(const char *dev, int gpt) { return syscall(uSYS_disk_table, (long)dev, gpt, 0, 0, 0, 0); }
int partition_create(const char *dev, unsigned long size_mb, int type) { return syscall(uSYS_part_create, (long)dev, (long)size_mb, type, 0, 0, 0); }
int partition_delete(const char *dev) { return syscall(uSYS_part_delete, (long)dev, 0, 0, 0, 0, 0); }
...
int symlink(const char *target, const char *linkpath) { return syscall(SYS_symlink, (long)target, (long)linkpath, 0, 0, 0, 0); }
int readlink(const char *path, char *buf, size_t size) { return syscall(SYS_readlink, (long)path, (long)buf, size, 0, 0, 0); }
...
struct linux_dirent64 *readdir(DIR *dirp) {
    ...
    if (dirp->buf_pos >= dirp->buf_end) {
        long res = syscall(SYS_getdents64, dirp->fd, (long)dirp->buf, sizeof(dirp->buf), 0, 0, 0);
        if (res <= 0) return NULL;
        dirp->buf_end = (int)res;
        dirp->buf_pos = 0;
    }
    struct linux_dirent64 *ent = (struct linux_dirent64*)(dirp->buf + dirp->buf_pos);
...
#define SYS_chroot 66

int chroot(const char *path) {
    return syscall(SYS_chroot, (long)path, 0, 0, 0, 0, 0);
}
