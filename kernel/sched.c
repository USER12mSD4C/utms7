...
// Вспомогательные функции очередей (без изменений)
static void enqueue_ready(process_t *p) {
    if (!p || p->state != PROC_READY) return;
    p->next = NULL;
    if (!ready_queue) { ready_queue = p; }
    else { process_t *last = ready_queue; while (last->next) last = last->next; last->next = p; }
}
...
static void remove_from_ready(process_t *p) {
    ...
    if (ready_queue == p) { ready_queue = ready_queue->next; p->next = NULL; return; }
    process_t *prev = ready_queue, *cur = ready_queue->next;
    while (cur) {
        if (cur == p) { prev->next = cur->next; p->next = NULL; return; }
...
static void enqueue_sleep(process_t *p) {
    ...
    if (!sleep_queue || p->sleep_until < sleep_queue->sleep_until) {
        p->next = sleep_queue; sleep_queue = p; return;
    }
    process_t *prev = sleep_queue, *cur = sleep_queue->next;
    while (cur && cur->sleep_until <= p->sleep_until) { prev = cur; cur = cur->next; }
    prev->next = p; p->next = cur;
}
static void remove_from_sleep(process_t *p) {
    if (!sleep_queue) return;
    if (sleep_queue == p) { sleep_queue = sleep_queue->next; p->next = NULL; return; }
    process_t *prev = sleep_queue, *cur = sleep_queue->next;
    while (cur) {
        if (cur == p) { prev->next = cur->next; p->next = NULL; return; }
        prev = cur; cur = cur->next;
...
static u32 alloc_pid(void) {
    u32 pid = next_pid++; if (next_pid >= 10000) next_pid = 1; return pid;
}
static process_t* find_free_proc(void) {
    for (int i = 0; i < MAX_PROCESSES; i++)
        if (processes[i].state == PROC_UNUSED) { memset(&processes[i], 0, sizeof(process_t)); return &processes[i]; }
    return NULL;
}
static void free_process(process_t *p) {
    if (!p) return;
    if (p->kstack) { kfree((void*)p->kstack); p->kstack = 0; }
    if (p->cr3 && p->cr3 != (u64)0x1000) { free_address_space((u64*)p->cr3); p->cr3 = 0; }
...
void sched_init(void) {
    ...
    current = NULL; next_pid = 1; process_count = 0;
    ...
    frame->cs = 0x08; frame->rflags = 0x202; frame->rsp = idle->kstack_top; frame->ss = 0x10;
    idle->kstack_top = (u64)frame;

    for (int i = 0; i < 32; i++) idle->fds[i].used = 0;
    enqueue_ready(idle);
    current = idle; process_count = 1;
}
...
int sched_create_kthread(const char* name, void (*entry)(void*), void* arg) {
    ...
    strncpy(p->name, name, 31); p->state = PROC_READY; p->ticks_left = TIME_SLICE;
    ...
    frame->cs = 0x08; frame->rflags = 0x202; frame->rsp = (u64)frame; frame->ss = 0x10;
    u64 *stack = (u64*)frame;
...
void sched_start(void) {
    ...
    if (!first) return;
    first->state = PROC_RUNNING; first->ticks_left = TIME_SLICE; current = first;
    __asm__ volatile (
        "movq %0, %%rsp\n"
        "movq %1, %%cr3\n"
        "popq %%r15\n popq %%r14\n popq %%r13\n popq %%r12\n popq %%r11\n popq %%r10\n popq %%r9\n popq %%r8\n"
        "popq %%rbp\n popq %%rdi\n popq %%rsi\n popq %%rdx\n popq %%rcx\n popq %%rbx\n popq %%rax\n"
        "iretq\n"
        : : "r"(first->kstack_top), "r"(first->cr3) : "memory"
    );
}

process_t* sched_pick_next(void) {
    if (!current) return NULL;
    if (current->state == PROC_RUNNING) { current->state = PROC_READY; enqueue_ready(current); }
    u32 now = get_ticks();
    while (sleep_queue && sleep_queue->sleep_until <= now) {
        process_t *p = sleep_queue; sleep_queue = sleep_queue->next; p->next = NULL;
        p->state = PROC_READY; enqueue_ready(p);
    }
    process_t *next = dequeue_ready();
    if (!next) next = current;
    next->state = PROC_RUNNING; next->ticks_left = TIME_SLICE;
    current = next;
...
void sched_exit(int code) {
    ...
    current->exit_code = code; current->state = PROC_ZOMBIE;
    if (current->waiting_for) { current->waiting_for->state = PROC_READY; enqueue_ready(current->waiting_for); }
    free_process(current); process_count--;
    sched_need_resched = 1;
    __asm__ volatile ("sti; hlt; cli");
    while (1) __asm__ volatile ("hlt");
}

void sched_yield(void) { sched_need_resched = 1; __asm__ volatile ("sti; hlt; cli"); }
void sched_sleep(u32 ms) {
    if (!current || current->pid == 1) return;
    u32 now = get_ticks(); current->sleep_until = now + ms; current->state = PROC_SLEEPING;
    remove_from_ready(current); enqueue_sleep(current);
    sched_need_resched = 1; __asm__ volatile ("sti; hlt; cli");
}
...
int sched_get_processes(process_t** buf, int max) {
    int count = 0;
    for (int i = 0; i < MAX_PROCESSES; i++) if (processes[i].state != PROC_UNUSED) count++;
    if (buf && max > 0) {
        int idx = 0;
        for (int i = 0; i < MAX_PROCESSES && idx < max; i++)
            if (processes[i].state != PROC_UNUSED) buf[idx++] = &processes[i];
    }
    return count;
}
int sched_kill(int pid) {
    ...
    for (int i = 0; i < MAX_PROCESSES; i++)
        if (processes[i].pid == (u32)pid && processes[i].state != PROC_ZOMBIE) {
            processes[i].state = PROC_ZOMBIE; processes[i].exit_code = -1;
            free_process(&processes[i]); process_count--; return 0;
        }
    return -1;
}
...
int sched_waitpid(u32 pid, int* status) {
    ...
    for (int i = 0; i < MAX_PROCESSES; i++) if (processes[i].pid == pid) { target = &processes[i]; break; }
    if (!target) return -1;
    if (target->state == PROC_ZOMBIE) {
        if (status) *status = target->exit_code; target->state = PROC_UNUSED; process_count--; return pid;
    }
    current->waiting_for = target; current->state = PROC_BLOCKED;
    sched_need_resched = 1; __asm__ volatile ("sti; hlt; cli");
    if (status) *status = target->exit_code; target->state = PROC_UNUSED; process_count--;
    return pid;
}
